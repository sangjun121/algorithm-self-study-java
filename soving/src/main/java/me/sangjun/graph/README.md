...
## BOJ2178 미로 탐색
- 마찬가지로 BFS의 성질을 활용하여, [최소 이동 횟수를 구하는 문제](../../../../../../../study/graph/README1.md)
- 이 문제를 그래프로 해석할 수 있냐도 중요하다.

## BOJ3055 탈출
- 해당 문제의 핵심 키포인트는 각 칸에 몇초 안에 물이 도착하는지를 먼저 계산하고, 해당 위치를 고슴도치가 이동하는 그 순간(t초)에 지나갈 수 있는지 판단하는 로직이다.
- 문제 풀이시 막힌 점: 물이 애초에 가지 못하는 위치의 경우 waterDist가 0으로 표기되어 있어, 고슴도치도 이동하지 못하는 것으로 인식되었다. 이 때문에 고슴도치 굴도 가지 못하는 것으로 설정되어 문제 발생.
