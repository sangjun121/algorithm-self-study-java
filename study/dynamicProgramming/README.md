...
- 예를 들어, "i~j사이 구간의 최소합"을 구하는 서브 문제를 구성. 이에 따라 진짜 문제는 0~N(마지막 구간)까지의 최소합을 구하는 문제로 정의 될 수 있다.   
- 예시: BOJ11066 파일 합치기
  - 파일 합산시에 연속성이 중요하고, 따라서 구간으로 치환하여 떠올리는 아이디어.

## 유형4. 2차원 격자 배열에서 문제를 해결할 때
## 유형5. 트리 구조에서 문제를 해결할 때
- 빈출유형!
- 개념: 큰 서브트리 문제를 해결하기 위해, 작은 서브트리를 사용하는 유형
- 예를 들어, Dy[i] 는 i를 루트로 하는 서브트리의 정점의 개수. 초기값은 단말노드(정점 1개).
- 예시 문항: BOJ15681 트리와 쿼리
- Rooted Tree문제를 DP로 해결할 경우, 대부분 DFS 한번에 해결 가능하다. 따라서 DFS의 시간복잡도인 O(v+e) = O(n)만에 문제를 해결할 수 있다.
